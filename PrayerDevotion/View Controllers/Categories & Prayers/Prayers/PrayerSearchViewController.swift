...
class SearchViewController: UITableViewController, UISearchResultsUpdating, UISearchBarDelegate {
    ...
    func updateSearchResultsForSearchController(searchController: UISearchController) {
        ...
        let prayers = PrayerStore.sharedInstance.filterPrayers(searchText: searchText, sortDescriptors: [NSSortDescriptor(key: "name", ascending: true)], batchSize: 50)
    ...
    override func tableView(tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        print("Filtered Prayers Count = \(filteredPrayers.count)")
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCellWithIdentifier(PrayerSearchCellID, forIndexPath: indexPath) as! PrayerSearchCell
        
        let currentPrayer = filteredPrayers[indexPath.row]
        
        print("Prayer \(currentPrayer.name) is at indexPath \(indexPath) and is in category \(currentPrayer.category)")
    ...
    func reloadFromEditingPrayer(notification: NSNotification) {
        ...
        let searchString = newSearchController.searchBar.text!
        
        let prayers = PrayerStore.sharedInstance.filterPrayers(searchText: searchString, sortDescriptors: [NSSortDescriptor(key: "name", ascending: true)], batchSize: 50)
...
class PrayerSearchViewController: UITableViewController {
    ...
    func configureCell(cell: PrayerSearchCell, indexPath: NSIndexPath) {
        ...
        let prayer = filteredPrayers[indexPath.row] as! PDPrayer
